...
/*   Updated: 2022/05/15 20:50:00 by rwallier         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static	void	ft_writenbr(unsigned int nb, char *strnbr, size_t maxsize)
{
	...
	{
		character = nb + 48;
		ft_strlcat_nonnull(strnbr, &character, maxsize - ft_strlen(strnbr));
		return ;
	}
	else
...
char	*ft_itoa(int n)
{
	...
	if (n < 0)
	{
		ret = (char *)malloc(ret_lenght * sizeof(char) + 1 + 1);
		ret[0] = '-';
		nbr = n * -1;
		ret_lenght++;
	}
	...
	{
		ret = (char *)malloc(ret_lenght * sizeof(char) + 1);
		nbr = n;
	}
	printf("--%i--\n", nbr);
	if (!ret)
